...
function App({
  ...
  }, [map, nowUserPosition, marker, circle, userDataStatus]);
  ...
  const registerLocInform = (
    ...
    onCircle(
      returnCircle(map, circle.getCenter(), circle.getRadius(), circleColor)
    );
  ...
  const returnCircle = (circleMap, circleCenter, circleRad, circleColor) => {
  ...
  const selectPosition = () => {
    if (nowSettingPos === 'start') {
      ...
      setCircle(
        returnCircle(map, map.getCenter(), circle.getRadius(), '#FBBA00')
      );
    } else if (nowSettingPos === 'arrive') {
      registerLocInform(
        setArriveAddress,

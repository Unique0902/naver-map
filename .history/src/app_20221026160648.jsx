...
function App({
  ...
  useEffect(() => {
    if (roomId) {
      const stopSync2 = matchService.onDeleteSync((match) => {
  ...
  useEffect(() => {
    if (matchDataNum && matchId) {
      matchService.take(matchId).then((data) => {
        if (matchDataNum == 1) {
          findMatchingService.take(data.matchData2Id).then((matchData) => {
            makeOpponentData(matchData);
          });
        } else if (matchDataNum == 2) {
          findMatchingService.take(data.matchData1Id).then((matchData) => {
            makeOpponentData(matchData);
          });
        }
      });
